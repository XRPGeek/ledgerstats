...
if (fs.existsSync(__dirname + '/data/' + filename)) {
    ...
    console.log(' -- Total XAG existing:   ', numeral(data.stats.total_coins).format('0,0.000000'))
    exportJson.meta.existingXAG = data.stats.total_coins
    console.log('')
    console.log('Calculating spendable XAG sum')
    ...
    console.log('  > Balance percentage (vs. Total XAG existing)  ', numeral(top100sum / data.stats.total_coins * 100).format('0.00') + ' %')
    ...
    percentages.forEach(p => {
        ...
        pctAccountsBalance.push([ 
            ...
            numeral(e).format('0,0.000000') + ' XAG'
        ])
    ...
    let noAccountsBalanceRange = [ [ '# Accounts', 'Balance from', '... To', 'Sum (XAG)' ] ]
    let sliceFrom = 0
